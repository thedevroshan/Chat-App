...
export const GetUserInfo = async (req, res) => {
    try {
        ...
        res.status(200).json({ok: true, msg: `Info of ${req.user.id}`, data: user})
    } catch (error) {
...
export const ChangeEmailRequest = async (req, res)  => {
    try {
        ...
        const isSent = await CreateSendOTP(configuration.OTP_LENGTH, user.email, 'Email Change Request', `Here is your OTP to change your email. Note this otp will be valid for ${configuration.OTP_EXPIRATION_IN_MINUTES} minutes.`, configuration.OTP_EXPIRATION_IN_MINUTES)

        if(!isSent){
           return res.status(400).json({ok: false, msg: 'Sorry, There is some problem to send otp. Please try again later'})
        }
...
export const ChangeEmail = async (req, res)  => {
    try {
        ...
        if(!isUpated){
            return res.status(400).json({ok: false, msg: 'Unable to upated email.'})
        }

        await CreateSendOTP(configuration.OTP_LENGTH, email, 'Email Verification', `Here is your otp to verify your email. Note this email will be expired in ${configuration.OTP_EXPIRATION_IN_MINUTES} minutes.`,configuration.OTP_EXPIRATION_IN_MINUTES)
...
export const UpdateProfile = async (req, res) => {
    try {
        ...
        if(!isUpdated){
            return res.status(400).json({ok:false, msg: 'Unable to update profile'})
        }
...
export const ChangePassword = async (req, res) => {
    try {
        ...
        if(!isUpdated){
            return res.status(400).json({ok: false, msg: 'Unable to change password'})
        }
...
export const ForgotPassword = async (req, res) => {
    try {
        const user = await User.findOne({username: req.body.username_or_email}) || await User.findOne({email: req.body.username_or_email})

        if(!user){
            return res.status(404).json({ok: false,msg: 'User not found'})
        }

        await CreateSendOTP(configuration.OTP_LENGTH, user.email, 'Forgot Password', `Here is your otp to reset your password. Note this otp will be expired in ${configuration.OTP_EXPIRATION_IN_MINUTES} minutes.`, configuration.OTP_EXPIRATION_IN_MINUTES)
...
export const ResetPassword = async (req, res) => {
    try {
        ...
        if(req.body.password !== req.body.confirm_password){
            return res.status(400).json({ok: false, msg: 'Password and Confirm Password must be same.'})
        }

        const salt = await bcrypt.genSalt(configuration.SALT_LENGTH)
        const hashedPassword = await bcrypt.hash(req.body.confirm_password, salt)

        const isUpdated = await User.findOneAndUpdate({email: isOTPValidated.user}, {
            $set: {
                password: hashedPassword
            }
        })
        ...
        if(!isUpdated){
            return res.status(400).json({ok: false, msg: 'Unable to reset password'})
        }
...
export const AddLink = async (req, res) => {
    try {
        ...
        if(title == '' || url == ''){
            return res.status(400).json({ok: false, msg: 'URL and Title is required.'})
        }
...
export const ChangeProfilePic = async (req, res) => {
    try {
        ...
        if(req.body.length > 5 * 1024 * 1024){
            return re.status(400).json({ok: false, msg: 'File size is too big. 5MB is the max limit'})
        }
        ...
        if(!isUserUpdated) {
            return res.status(400).json({ok: false, msg: 'Unable to update profile pic'})
        }

        res.status(200).json({ok: true, msg: 'Profile Pic Changed', data: fileViewURL})
    } catch (error) {
        if(configuration.IS_DEV_ENV){
